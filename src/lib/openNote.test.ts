...
describe('followNote', () => {
  ...
  it('leaves a note on the change it was just opened for', () => {
    // Picking a letter in the gutter moves the cursor and opens that change:
    // the move must not then be read as a reason to change the note again.
    const note: NoteDialog = { kind: 'change', changeId: '1' };
    expect(followNote(note, at('h0', '0'), at('h1', '1'))).toBe(note);
  });
});
